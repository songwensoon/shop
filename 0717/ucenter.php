...
class Ucenter extends IController
{
	...
	public function userInfoRemind()
	{
		$flag = IFilter::act(IReq::get('flag'));
		$user_id = ISafe::get('user_id');
		if($flag=='apply_member'){
			$message = '请完善个人资料后再申请会员！';	
		}else if($flag=='order'){
			$message = '请完善个人资料后再下单！';
		}else{
			$this->redirect('/site/error/msg/当前要访问的文件不存在');
			return;
		}
	...
	public function apply_member()
	{
		...
		if(!$result){
			$this->redirect('/ucenter/userInfoRemind/flag/apply_member');
			return;
		}
		$siteConfigObj = new Config("site_config");
		$site_config   = $siteConfigObj->getInfo();
		$member_fee = $site_config['fees'];
		$this->fees = $member_fee;
	...
	function check_recommended($recommended=null)
	{
		...
		$userDB->where= 'u.username = "'.$o_recommended.'" and m.group_id in(2,3,4) and u.id != '.$user_id;
		$userRow = $userDB->find();
    ...
    function info_edit_act()
    {
    	...
    	$is_success = $memberObj->update($where);
