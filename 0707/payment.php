...
class Payment
{
	...
	public static function updateMembership($fees_no)
	{
		...
		$memberObj->setData($dataArray);
		$is_success = $memberObj->update('user_id = '.$user_id,array('membership_starttime','membership_endtime','group_id'));
	...
	public static function updateIntegral($recommended)
	{
		...
		$pointConfig = array(
			'user_id' => $user_id,
			'point'   => $recommended_points,
			'log'     => '成功推荐会员,奖励积分'.$recommended_points,
		);
		$pointObj = new Point();
		$is_success = $pointObj->update($pointConfig);

		if($is_success&&($count>=5)&&($group_id==2)){
			$m_array = array();
			$dataArray = array();
