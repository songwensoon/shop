...
class Block extends IController
{
    ...
    */
	public function doPay()
	{
		...
		$member_fee = IReq::get('member_fee');
		if($order_id)
		{
			//获取订单信息
		...
		else if(!empty($member_fee))
		{
			$user_groupObj = new IModel('user_group');
			$where = 'id = 2';
			$user_groupRow = $user_groupObj->getObj($where);
			$member_fee = $user_groupRow['fees'];
			$member_fee   = IFilter::act($member_fee,'float');
			$paymentRow = Payment::getPaymentById($payment_id);

			//account:充值金额; paymentName:支付方式名字
			$reData   = array('account' => $member_fee , 'paymentName' => $paymentRow['name']);
			$sendData = $paymentInstance->getSendData(Payment::getPaymentInfo($payment_id,'membershipfees',$reData));
		}else
		{
	...
	public function callback()
	{
		...
		if($return == 1)
		{
			...
			if(stripos($orderNo,'recharge_') !== false)
			{
				...
				//会员缴费方式
			}else if(stripos($orderNo,'membershipfees_') !== false)
			{
				$user_groupObj = new IModel('user_group');
				$where = 'id = 2';
				$user_groupRow = $user_groupObj->getObj($where);
				...
				$member_fee   = IFilter::act($member_fee,'float');
				if($member_fee!=$money){
					IError::show(403,'缴费失败,会员费用不一致');
					exit;
				}

				$tradenoArray = explode('_',$orderNo);
				$fees_no  = isset($tradenoArray[1]) ? $tradenoArray[1] : 0;
				if(payment::updateMembership($fees_no))
				{
					$group_id = ISafe::get('group_id');
					if($group_id == 1){
						ISafe::set('group_id',2);
					}
					$this->redirect('/site/success/message/'.urlencode("会员申请成功").'/?callback=/ucenter/account_log');
					exit;
				}
				IError::show(403,'缴费失败');
			}
			else
			{
