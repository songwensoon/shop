...
class Payment
{
	...
	public static function updateMembership($fees_no)
	{
		...
		$membership_time = $memberRow['membership_time'];
		
		if(!empty($membership_time)){
			$membership_time = strtotime("+1 years",$membership_time);
		}else{
			$membership_time = strtotime("+1 years");
		}
		$dataArray = array(
		...
		$memberObj->setData($dataArray);
		$is_success = $memberObj->update('user_id = '.$user_id,array('membership_time','group_id'));
